...
<img src="https://skillicons.dev/icons?i=react,nextjs,vuejs,spring&perline=7">
<h3>
Database
</h3>
...
Como Scrum Master, acompanhei o desenvolvimento com a equipe através de um burndown chart, enviando um relatorio diario para a equipe dando pequenas ideias do progresso da equipe no dia a dia.

<details>
  <summary><strong>Burndown Sprint 4 - Plataforma Scrum</strong></summary>
  
  <img width="538" height="295" alt="image" src="https://github.com/user-attachments/assets/884c9823-6542-4638-9f74-c62ccc7c618c" />
</details>
...
Como Scrum Master,  acompanhei o progresso da equipe durante as sprints utilizando um Burndown e o Trello onde as tarefas estavam organizadas por sprints e por prioridade.

<details>
  <summary><strong>Burndown Sprint 2 - WeCollab</strong></summary>
  
  <img width="593" height="330" alt="image" src="https://github.com/user-attachments/assets/19d0dcb3-6904-4d26-b45a-79815ebd8d7e" />
</details>
...
<details>
  ...
  | Prioridade | User Story                                                                                                                                                                            | Estimativa | Sprint | Requisito do Parceiro | CRITÉRIO DE ACEITAÇÃO                                                                                                                                                                                                          |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | ------ | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| ALTA       | "Como usuário, quero fazer login na aplicação, para que possa acessar minhas informações e funcionalidades."                                                                          | 2          | 1      | RF1                   | Tela com Campos para acesso a aplicação com  E-mail e senha para acesso<br>Validar as credenciais e permitir acesso à aplicação se forem corretas<br>Redirecionamento para Tela Principal ao logar com sucesso                 |
| ALTA       | "Como usuário, quero selecionar o tipo de reembolso que estou solicitando, para que possa categorizar corretamente minha despesa."                                                    | 2          | 1      | RF1                   | Usuário deve ser capaz de selecionar o tipo de reembolso de uma lista de tipos disponíveis<br>O tipo de reembolso solictiado deve aparecer na tela                                                                             |
| ALTA       | "Como usuário, quero informar o valor do reembolso, para que possa registrar minha solicitação de forma precisa."                                                                     | 1          | 1      | RF1                   | O campo do valor do reembolso só pode aceita números<br>O valor deve ser validado para certificar que é positivo e dentro do limite especificado                                                                               |
| ALTA       | "Como usuário, quero enviar uma foto do comprovante armazenada na minha galeria, para que possa anexá-la ao meu pedido de reembolso."                                                 | 3          | 1      | RF2                   | Usuário deve ser capaz de acessar sua galeria de fotos/arquivos do dispositivo<br>Usuário deve poder selecionar uma foto/pdf para anexo<br>A foto/pdf deve ser exibido na tela antes do envio                                  |
| ALTA       | "Como usuário, quero ver uma lista dos meus pedidos de reembolso, para que possa acompanhar minhas solicitações."                                                                     | 1          | 1      | RF5                   | Tela com uma lista de pedidos de reembolso do usuário<br>A lista deve conter informações basicas do pedido                                                                                                                     |
| ALTA       | "Como usuário, quero receber um alerta ao tentar solicitar um reembolso acima do limite permitido, para que eu possa justificar o valor."                                             | 1          | 1      | RF4                   | Um alerta deve aparecer na tela do usuário caso ele ultrapasse o limite<br>O alerta deve fornecer informações de como justificar a ultrapassagem do limite                                                                     |
| ALTA       | "Como sistema, quero salvar a data, o valor e o tipo do reembolso, para que possa processar corretamente as solicitações dos usuários."                                               | 2          | 1      | RF6                   | Armazenar a data da solicitação, o valor e o tipo de reembolso corretamente no banco de dados<br>Essas informações devem ser acessiveis através de um endpoint na API                                                          |
| ALTA       | "Como usuário, quero adicionar uma descrição detalhada à minha despesa, para que possa justificar a solicitação de reembolso."                                                        | 1          | 1      | RF3                   | O campo de descrição permite ao usuário inserir texto<br>O campo de descrição é validado para garantir que o texto não seja vazio caso o usuário ultrapasse o limite de valor do pedido                                        |
| ALTA       | "Como usuário, quero poder excluir um comprovante anexado antes do envio, para que possa substituir por outro caso tenha anexado o errado."                                           | 1          | 1      | RF5                   | O usuário pode excluir um comprovante anexado antes de finalizar a solicitação de reembolso<br>O sistema confirma a exclusão do arquivo com uma mensagem e retirando a imagem/pdf da tela                                      |
| ALTA       | "Como usuário, quero pertencer a múltiplos grupos e projetos, para que possa gerenciar reembolsos de diferentes áreas."                                                               | 3          | 2      | RF1                   | O sistema deve associar o usuário a grupos/projetos, cada projeto pode ter valores limites e opções diferentes para o usuário.                                                                                                 |
| ALTA       | "Como usuário, quero escolher o projeto, grupo ou área do reembolso, para que possa organizar minhas solicitações corretamente."                                                      | 3          | 2      | RF1                   | O usuário deve ser capaz de escolher um projeto em específico para realização do reembolso.<br>O sistema deve listar os projetos que o usuário se mostra habilitado para a realização do reembolso e dar a opção para escolha. |
| ALTA       | "Como usuário, quero tirar uma foto do comprovante de despesa, para que possa anexá-lo à minha solicitação de reembolso."                                                             | 2          | 2      | RF2                   | O usuário deve ser capaz de escolher a utilizar a câmera do dispositivo para enviar comprovantes como anexos.                                                                                                                  |
| ALTA       | "Como usuário, quero visualizar o status do meu pedido de reembolso, para que possa saber se foi aprovado ou rejeitado."                                                              | 1          | 2      | RF5                   | O usuário deve ter acesso a uma lista de reembolsos em que seus status são apresentados.<br>O sistema deve só apresentar reembolsos relevantes ao usuário.                                                                     |
| MÉDIA      | "Como usuário, quero que meus reembolsos sejam organizados por projeto, grupo ou área, para que possa gerenciá-los com mais eficiência."                                              | 3          | 2      | RF1                   | O usuário deve poder filtrar seus reembolsos por projeto através de um botão com uma lista dos projetos da qual ele pertence.<br>Os projetos apresentados devem ser relevantes ao usuário/permitidos a ele                     |
| MÉDIA      | "Como usuário, quero cancelar uma solicitação de reembolso antes da aprovação, para que possa evitar processamento de pedidos incorretos."                                            | 1          | 2      | RF5                   | O usuário deve ser apresentado com um botão para cancelar seu pedido antes do envio do mesmo.<br>O sistema deve confirmar a exclusão do pedido para que não haja a possibilidade de processamento errôneo.                     |
| MÉDIA      | "Como usuário, quero filtrar meus reembolsos por status (pendente, aprovado, rejeitado) ou por período (mensal, trimestral, anual), para que possa encontrar informações rapidamente" | 4          | 2      | RF5                   | O usuário deve poder escolher em um botão o método de filtragem por status do pedido.                                                                                                                                         |
| MÉDIA      | "Como desenvolvidor, quero acesso a uma documentação ampla da funcionalidade dos endpoints do sistema."                                                                               | 5          | 3      | RNF2                  | O sistema deve disponibilizar acesso a uma página web (swagger) com documentação de todos os end-points da API utilizada na aplicação com exemplos.                                                                            |
| MÉDIA      | "Como usuário, quero acesso a um manual de uso e instalação da aplicação."                                                                                                            | 3          | 3      | RNF1                  | O  README do projeto deve incluir instruções de como rodar o projeto                                                                                                                                                           |
| MÉDIA      | "Como usuário, quero ver o motivo pelo qual meu reembolso foi rejeitado, para que possa corrigir e reenviar, se necessário."                                                          | 1          | 3      | RF5                   | O projeto deve mostrar dentro de uma tela separada detalhes do estado do reembolso e a justificativa da rejeição caso haja.                                                                                                    |
</details>
